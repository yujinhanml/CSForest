...
BCOPS.loop<-function(n,n_train,n_test,n_test_outlier,mydata='mnist',networkD=NA,train.label=NA,test.label=NA,trainData.full=NA,testData.full=NA,alpha=0.05,K=5,seed=T,type='exp1',B=3000){
  averagetypeI = list()
  averagetypeII = list()
  ...
  for (round in 1:n) {
    ...
    #cat('\nDim of test data',dim(x.testing))
    
    bcops = BCOPS(ranger, xtrain1, ytrain1, xtest1, xtrain2, ytrain2, xtest2, labels, formula = TRUE, prediction_only = FALSE,num.trees = B)
    Cscores = matrix(NA, ncol = length(labels), nrow = length(ytest))
    Cscores[foldid_te==1,] = bcops[[1]]
    Cscores[foldid_te==2,] = bcops[[2]]
